...
//			AbstractUpdateService, ClockWidget_small

public class UpdateService_small extends AbstractUpdateService
{

	public UpdateService_small()
	{
	}
	...
	protected String getDateFormatString(String s, Date date, boolean flag)
	{
		...
		if (s.equals("EE, MM dd"))
			return (new StringBuilder(String.valueOf(getString(getShortDayofWeek(date.getDay()))))).append(", ").append(getString(getMonth(date.getMonth()))).append(" ").append(s1).toString();
		if (s.equals("EE, dd MM"))
			return (new StringBuilder(String.valueOf(getString(getShortDayofWeek(date.getDay()))))).append(", ").append(s1).append(" ").append(getString(getMonth(date.getMonth()))).toString();
	...
	{
		AppWidgetManager.getInstance(this).updateAppWidget(new ComponentName(this, ClockWidget_small.class), remoteviews);
	}
}
